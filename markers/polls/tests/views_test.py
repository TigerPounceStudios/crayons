...
def test_poll_endpoint(rest_client, sample_user, poll_factory, sample_full_poll_data_with_responses):
    poll1 = poll_factory()
    poll2 = poll_factory()

    rest_client.force_authenticate(user=sample_user)
...
@pytest.mark.integration
def test_responses_view_returns_all_possible_responses(user_factory, sample_full_poll_data_with_responses):
    user = user_factory()
...
@pytest.mark.integration
def test_can_add_response_via_url(client, user_factory, sample_full_poll_data_no_responses):
    bob = user_factory(username="bob", password="orcpass")
